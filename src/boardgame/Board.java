...
public class Board {
	...
	public Board(int rows, int columns) {
		if(rows < 1 || columns < 1) throw  new BoardException(
	...
	public Piece piece(int row, int column) {
		if(!positionExistis(row,column)) throw new BoardException("error: position not found");
		return pieces[row][column];
	}

	public Piece piece(Position position) {
		if(!positionExistis(position.getRow(),position.getColumn())) throw new BoardException("error: position not found");
		return pieces[position.getRow()][position.getColumn()];
	}

	public void placePiece(Piece piece, Position position) {
		
		if(thereIsAPiece(position)) throw new BoardException("error: there is already a piece on position: "+ position);
